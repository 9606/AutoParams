...
opt_algo_params = '[{"name":"bo_loop","val":20,"placeholder":"迭代次数","isNumber":true},{"name":"bo_random_choice","val":5,"placeholder":"先验个数","isNumber":true},{"name":"bo_count","val":1,"placeholder":"迭代选择个数","isNumber":true}]'
...
if algor_label[1] == '1':
    if json.loads(other_params)["fileType"] == "csv":
        file_path = file_path_prefix + "classfication/dataset.csv"
    else:
...
if sys_str == "Windows":
    # pargs = [INTERPRETER, processor, algorithm_type, algor_params_str.replace('"', '"""'), json.dumps(other_params_json).replace('"', '"""'), 'Bayesian_Optimization', opt_algo_params.replace('"', '"""')]
